...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand(sc => sc
      ...
      .addStringOption(o => o.setName('role_id').setDescription('Rôle requis (ID)'))
      ...
      .addStringOption(o => o.setName('exempt_role_ids').setDescription('IDs de rôles exemptés, séparés par des virgules'))
    )
    .addSubcommand(sc => sc
  ...
  async execute(interaction, dataManager) {
    ...
    try {
      ...
      if (sub === 'config') {
        ...
        const exemptRoleIds = (interaction.options.getString('exempt_role_ids') || '').split(',').map(s => s.trim()).filter(Boolean);
        ...
        if (roleId) updates.roleEnforcement = { ...(updates.roleEnforcement || {}), requiredRoleId: roleId };
        ...
        if (exemptRoleIds.length > 0) updates.inactivity = { ...(updates.inactivity || {}), exemptRoleIds };

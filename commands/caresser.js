...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('caresser')
    .setDescription('Caresser un membre (NSFW)')
  ...
  async execute(interaction, dataManager) {
    try {
      ...
      const cfg = (economyConfig.actions?.caresser) || {};
      const enabled = cfg.enabled !== false;
      const cooldown = n(cfg.cooldown, 900_000); // 15 min
      const min = n(cfg.minReward, 15);
      const max = n(cfg.maxReward, 50);
      const gK = n(cfg.goodKarma, 1);
      const bK = n(cfg.badKarma, 0);

      if (!enabled) {
        return await interaction.reply({ content: '❌ La commande /caresser est désactivée.', flags: 64 });
      }

      const userData = await dataManager.getUser(userId, guildId);
      const now = Date.now();
      if (userData.lastCaresser && (now - userData.lastCaresser) < cooldown) {
        const remaining = Math.ceil((cooldown - (now - userData.lastCaresser)) / 60000);
      ...
      userData.lastCaresser = now;
      await dataManager.updateUser(userId, guildId, userData);

      const karmaNet = (userData.goodKarma || 0) + (userData.badKarma || 0);
      const embed = new EmbedBuilder()
        .setColor('#fab1a0')
        .setTitle('🤲 Douce Caresse')
        .setDescription(`Vous avez caressé <@${target.id}> et gagné **+${gain}💋**`)
        .addFields(
      ...
      console.error('❌ Erreur caresser:', error);

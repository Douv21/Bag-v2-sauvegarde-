...
module.exports = {
    ...
    async execute(interaction) {
        try {
            ...
            try {
                console.log(`🖼️ Téléchargement avatar: ${targetUser.displayAvatarURL({ format: 'png', size: 128 })}`);
                avatarBase64 = await this.downloadAvatar(targetUser.displayAvatarURL({ format: 'png', size: 128 }));
                if (avatarBase64) {
                    console.log(`✅ Avatar téléchargé: ${avatarBase64.substring(0, 50)}...`);
                }
            } catch (error) {
                console.error('❌ Erreur avatar:', error);
            }
            
            // Créer SVG avec avatar
            const svgCard = this.createSVGWithAvatar(targetUser, userData, {
                karmaNet, karmaLevel, cardRarity, discordDate, serverDate, avatarBase64
            });
    ...
    createSVGWithAvatar(user, userData, stats) {
        const { karmaNet, karmaLevel, cardRarity, discordDate, serverDate, avatarBase64 } = stats;
...
<svg width="500" height="800" xmlns="http://www.w3.org/2000/svg">
  ...
  <image x="210" y="160" width="80" height="80" 
  ...
  <text x="250" y="260" text-anchor="middle" fill="#ffffff" 
        font-family="Arial, sans-serif" font-size="24" font-weight="bold">${userName}
  ...
  <text x="250" y="365" text-anchor="middle" fill="url(#goldGradient)" 
        font-family="Arial, sans-serif" font-size="20" font-weight="bold">${balance}€
  ...
  <text x="160" y="435" text-anchor="middle" fill="#00ff00" 
        font-family="Arial, sans-serif" font-size="16">😇 ${userData.goodKarma}
    <animate attributeName="opacity" values="0.7;1;0.7" dur="1.8s" repeatCount="indefinite"/>
  </text>
  <text x="340" y="435" text-anchor="middle" fill="#ff6666" 
        font-family="Arial, sans-serif" font-size="16">😈 ${userData.badKarma}
  ...
  <text x="120" y="525" fill="#9370DB" font-family="Arial, sans-serif" font-size="14">Discord: ${discordDate}
    <animate attributeName="opacity" values="0.6;1;0.6" dur="3.2s" repeatCount="indefinite"/>
  </text>
  <text x="120" y="550" fill="#9370DB" font-family="Arial, sans-serif" font-size="14">Serveur: ${serverDate}
  ...
  <text x="250" y="615" text-anchor="middle" fill="#00FF00" 
        font-family="Arial, sans-serif" font-size="20" font-weight="bold">${userData.dailyStreak} jours
    <animate attributeName="opacity" values="0.7;1;0.7" dur="2s" repeatCount="indefinite"/>
  </text>

  <!-- Rareté de carte -->
  <text x="250" y="680" text-anchor="middle" fill="${cardRarity.color}" 
        font-family="Arial, sans-serif" font-size="18" font-weight="bold">${cardRarity.icon} ${cardRarity.name}
  ...
  <rect x="80" y="0" width="340" height="3" fill="url(#rarityGradient)" opacity="0.6">
    ...
    async createCanvas_OLD(user, userData, stats) {
        ...
        const bgGradient = ctx.createRadialGradient(250, 400, 0, 250, 400, 600);
        ...
        ctx.lineWidth = 2;
        ctx.strokeRect(10, 10, 480, 780);

        // Photo de profil avec Canvas
        try {
            console.log(`🖼️ Chargement avatar: ${user.displayAvatarURL({ format: 'png', size: 128 })}`);
            ...
            ctx.clip();
            ctx.drawImage(avatar, 210, 160, 80, 80);
            ctx.restore();
            
            // Bordure avatar
            ctx.strokeStyle = cardRarity.color;
            ctx.lineWidth = 3;
            ctx.beginPath();
            ctx.arc(250, 200, 42, 0, Math.PI * 2);
            ctx.stroke();
            
            console.log(`✅ Avatar intégré avec succès`);
        } catch (error) {
            console.error('❌ Erreur avatar:', error);
            // Fallback: icône utilisateur
